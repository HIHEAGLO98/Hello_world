...
mil.use('TkAgg') # add it before lauching matplotlib
...
class Zone:
    ...
    def find_zone_that_contains(cls, position):
        #compute the index in the ZONES array that contains
        if not cls.ZONES:
            cls._initialize_zones()
        longitude_index = int((position.longitude_degrees - cls.MIN_LONGITUDE_DEGREES )/cls.HEIGHT_DEGREES)
        latitude_index = int((position.latitude_degrees - cls.MIN_LATITUDE_DEGREES)/cls.WIDTH_DEGREES)
        longitude_bins = int((cls.MAX_LONGITUDE_DEGREES - cls.MIN_LONGITUDE_DEGREES)/cls.WIDTH_DEGREES) #180-(-180) /1
        zone_index = latitude_index * longitude_bins + longitude_index

        # just cheking that index is correct
...
def main():
    ...
    #grap initialization
    agreeableness_grap = AgreeablenessGraph()
    #show graph
    agreeableness_grap.show(zone.ZONES)
main()
